defmodule AgentYardWeb.RunsLive.Show do
  ...
  def render(assigns) do
    ...
    <div class="metric-grid detail-metrics">
      ...
      <div class="metric-card"><span>Cost</span><strong>$<%= Decimal.to_string(@run.cost_usd || Decimal.new("0")) %></strong></div>
    ...
    <div class="detail-layout">
        ...
        <div class="timeline">
          <div :for={event <- @events} class={"timeline-event event-#{event.kind}"}>
            ...
            <div class="event-body">
              ...
              <pre :if={event.kind in ["tool_call", "tool_result"]}><%= inspect(event.payload["input"] || event.payload["output"] || event.payload["text"], pretty: true) %></pre>
      ...
      <aside class="stack">
        ...
        <section class="panel usage-card"><div class="panel-heading"><h2>Usage</h2><span class="muted">this run</span></div><dl class="detail-list"><div><dt>Input</dt><dd><%= @run.input_tokens || 0 %> tok</dd></div><div><dt>Output</dt><dd><%= @run.output_tokens || 0 %> tok</dd></div><div><dt>Budget</dt><dd>$<%= Decimal.to_string(@run.session.agent_profile.budget_usd || Decimal.new("5.00")) %></dd></div></dl><div class="progress"><span style="width: 38%"></span></div></section>
