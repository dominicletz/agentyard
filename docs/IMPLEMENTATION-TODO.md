...
| M08 | Apply CPU, memory, default-deny network, and allowlist hook settings | Done |
...
| M30 | Check mentioner write permission and document token-based App boundary | Done |
...
| M42 | Show a real workspace diff | Done |
...
| ID | MVP item | Status |
|---|---|---|
| M51 | Add a pinned runner image skeleton for Claude Code and Cursor CLI | Done |
| M52 | Test scoped secret injection and redaction | Done |
| M53 | Test forge orchestration with provider stubs | Done |
| M54 | Test Oban enqueue and worker behavior | Done |
| M55 | Test budget hard-stop behavior | Done |
| M56 | Test webhook signature verification | Done |
| M57 | Keep the README boundaries, Apache-2.0 license, and PR checklist current | Done |
...
dispatch, fail-closed fork/permission policy checks, and best-effort issue/PR
